...
import { Building2, Home, Calendar, LogOut, Menu, X, Search, User } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background">
      ...
      <aside className={cn("fixed top-0 left-0 z-50 h-full w-64 bg-card border-r border-border transform transition-transform duration-200 lg:translate-x-0", sidebarOpen ? "translate-x-0" : "-translate-x-full")}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-border">
          <div className="mb-4 px-4"><p className="text-sm font-medium">{userProfile?.name || 'Tenant'}</p><p className="text-xs text-muted-foreground">{userProfile?.email}</p></div>
